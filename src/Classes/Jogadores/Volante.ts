...
export default class Volante extends Jogador implements IJogadores {
	public static numIntancias: number = 0
	public static numTentativas: number = 0
	public static readonly MINIMO: number = 2
	public static readonly LIMITE: number = 4
	...
	constructor(jogador: IJogadores) {
		...
		try {
			if (Volante.numIntancias < Volante.LIMITE) {
				Volante.numIntancias++
			} else {
				throw new Error('!!!FALHA NO CADASTRO!!!\n\nLimite de Volantes excedido.')
			}
		} catch (erro) {
			console.log(erro.message)
		} finally {
			Volante.numTentativas++
		}
	}
	...
	public static override insertJogador() {
		...
		let volante: IJogadores = {
			...jogador,
			_usarMaos: false,
			_posicaoCampo: posicoesCampo.P4,
