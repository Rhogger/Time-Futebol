...
export default class Atacante extends Jogador implements IJogadores {
	...
	// Precisa ter uma boa visão do que ta acontecendo no campo?
	...
	constructor(jogador: IJogadores) {
		...
		try {
			if (Atacante.numIntancias < Atacante.LIMITE) {
				Atacante.numIntancias++
			} else {
				throw new Error('!!!FALHA NO CADASTRO!!!\n\nLimite de Atacantes excedido.')
			}
		} catch (erro) {
			console.log(erro.message)
		} finally {
			Atacante.numTentativas++
		}
	}
	...
	public static override insertJogador() {
		...
		let atacante: IJogadores = {
			...jogador,
			_usarMaos: false,
			_posicaoCampo: posicoesCampo.P5,
			_funcaoCampo: funcoesCampo.F1,
			_marcacao: true,
			_visaoDeJogo: true,
			_bomDeDrible: true,
			_finalizacao: true,
